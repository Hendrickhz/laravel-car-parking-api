...
class VehicleTest extends TestCase
{
    ...
    public function testUserCanGetTheirOwnVehicles()
    {
        ...
        $vehicleForJohnDoe = Vehicle::factory()->create([
            'user_id' => $johnDoe->id
        ]);
        ...
        $response = $this->actingAs($johnDoe)->getJson('/api/v1/vehicles');

        $response->assertStatus(200)
            ->assertJsonStructure(['data'])
            ->assertJsonCount(1, 'data')
            ->assertJsonPath('data.0.plate_number', $vehicleForJohnDoe->plate_number)
